...
function CampSiteItem({ siteInfo }: CampSiteItemProps) {
  ...
  return (
    <div className='flex flex-col gap-12pxr mobile:px-20pxr'>
      <div className='flex h-32pxr justify-between'>
        <h4 className='text-gray600 font-title3-semibold'>{campingZoneName}</h4>
        <span className='flex items-end gap-2pxr text-second100 font-caption1-semibold'>
          상세정보
          <IconTest />
        </span>
      </div>
      <div className='flex flex-col gap-12pxr bg-white p-16pxr'>
        ...
        <div className='ml-auto'>
          <Button.Round
            size='sm'
            custom={`w-98pxr !h-36pxr font-caption1-semibold text-white px-24pxr py-8xr`}
          >
            선택하기
          </Button.Round>
        </div>
